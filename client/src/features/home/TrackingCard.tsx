...
export default function TrackingCard({ delivery }: Props) {
  ...
  return <article className="rounded-2xl border border-sky-100 bg-white shadow-card dark:border-night-line dark:bg-night-1 dark:shadow-none"><button type="button" aria-expanded={open} onClick={() => setOpen((value) => !value)} className="flex w-full items-center gap-3 p-4 text-left"><div className="min-w-0 flex-1"><b className="block text-control">{destination}</b><small className="mt-0.5 block text-caption text-ink-muted dark:text-zinc-500">{method === 'BIRD' ? 'Pigeon en route' : 'Lettre en route'} · arrivée dans {remaining(delivery.delivered_at)}</small></div><IconChevronDown className={'shrink-0 text-lg text-ink-muted transition-transform dark:text-zinc-500 ' + (open ? 'rotate-180' : '')} /></button>{open && <div className="px-4 pb-4"><div className="h-40 overflow-hidden rounded-xl"><MappyPlanisphere msg={msg} state={state} distance={delivery.distance_km} showDistance={false} /></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-sky-100 dark:bg-night-2"><div className="h-full rounded-full bg-accent transition-[width] duration-700" style={{ width: `${progress * 100}%` }} /></div><div className="mt-2 flex justify-between text-nav text-ink-muted dark:text-zinc-500"><span>Parti {dateTime(delivery.sent_at)}</span><span>{Math.round(progress * 100)} %</span></div><div className="mt-3 rounded-xl bg-sky-50 px-3 py-2.5 text-caption dark:bg-night-2"><b className="block text-ink dark:text-zinc-100">Itinéraire</b><p className="mt-0.5 text-ink-muted dark:text-zinc-500">{method === 'BIRD' ? `Vous → ${destination} · ${Math.round(delivery.distance_km).toLocaleString('fr-FR')} km${timeline.effectiveSpeedKmH ? ` · ${timeline.effectiveSpeedKmH} km/h` : ''}` : `Vous → collecte → acheminement local → ${destination}`}</p></div></div>}</article>
}
