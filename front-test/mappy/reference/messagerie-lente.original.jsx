...
function birdState(plan, msg, now) {
  ...
  return {
    status: current === "FLY" ? "Rouge-gorge en vol" : "Rouge-gorge au repos",
    resting: current === "REST",
    kmLeft: Math.round(plan.eff * (1 - p)),
    pos: { lat: msg.from.lat + (msg.to.lat - msg.from.lat) * p, lon: msg.from.lon + (msg.to.lon - msg.from.lon) * p },
  };
}
...
function postState(plan, msg, now) {
  ...
  if (e.from && e.to) {
    ...
    pos = { lat: e.from.lat + (e.to.lat - e.from.lat) * p, lon: e.from.lon + (e.to.lon - e.from.lon) * p };
...
function WorldMap({ msg, state }) {
  const P = (pt) => ({ x: pt.lon + 180, y: 90 - pt.lat });
  const hubs = msg.method === "POST" ? msg.plan.route : [];
  const nodes = msg.method === "BIRD" ? [msg.from, msg.to] : [msg.from, ...hubs, msg.to];
  const path = nodes.map(P);
  const cur = P(state.pos);
  const a = P(msg.from), b = P(msg.to);

  /* cadrage : viewBox 16/9 centré, jamais trop zoomé */
  const xs = path.map((p) => p.x), ys = path.map((p) => p.y);
  const cx = (Math.min(...xs) + Math.max(...xs)) / 2;
  const cy = (Math.min(...ys) + Math.max(...ys)) / 2;
  const spanX = Math.max(...xs) - Math.min(...xs);
  const spanY = Math.max(...ys) - Math.min(...ys);
  const w = Math.max(spanX * 1.5, spanY * 1.6 * (16 / 9), 55);
  const h = (w * 9) / 16;
  const x0 = cx - w / 2, y0 = cy - h / 2;

  /* tailles proportionnelles au zoom — rendu constant */
  const F = w * 0.032;      // police
  const R = w * 0.011;      // rayon point
  const OFF = F * 0.7;

  /* étiquettes avec évitement de collisions + bords */
  const placed = [];
  const estW = (t) => t.length * F * 0.62;
  const mkLabel = (key, p, text, fill) => {
    let anchor = "start", dx = OFF;
    if (p.x + OFF + estW(text) > x0 + w - F * 0.6) { anchor = "end"; dx = -OFF; }
    const cands = [F * 0.38, -F * 0.85, F * 1.55, -F * 2.05, F * 2.75];
    let dy = cands[0];
    for (const c of cands) {
      const bx = anchor === "start" ? p.x + dx : p.x + dx - estW(text);
      const box = { x: bx, y: p.y + c - F, w: estW(text), h: F * 1.25 };
      const hit = placed.some((o) => box.x < o.x + o.w && box.x + box.w > o.x && box.y < o.y + o.h && box.y + box.h > o.y);
      if (!hit) { dy = c; placed.push(box); break; }
      dy = c;
    }
    const y = Math.min(Math.max(p.y + dy, y0 + F * 1.2), y0 + h - F * 0.5);
    return (
      <text key={key} x={p.x + dx} y={y} textAnchor={anchor} fontSize={F} fill={fill}
        fontFamily="ui-monospace,monospace" style={{ letterSpacing: F * 0.04 }}>{text}</text>
    );
  };

  const destLabel = msg.to.label.includes(",") ? "CIBLE" : msg.to.label.toUpperCase();
  const labels = [mkLabel("moi", a, "MOI", "#EDE6D2"), mkLabel("dest", b, destLabel, "#EDE6D2")];
  hubs.forEach((hub) => {
    const p = P(hub);
    if (Math.hypot(p.x - b.x, p.y - b.y) > F * 0.9 && Math.hypot(p.x - a.x, p.y - a.y) > F * 0.9)
      labels.push(mkLabel(hub.id, p, hub.id.slice(3), "#8FA2EC"));
  });

  /* graticule adapté au zoom */
  const step = [5, 10, 15, 30].find((s) => w / s <= 9) || 30;
  const grid = [];
  for (let gx = Math.ceil(x0 / step) * step; gx < x0 + w; gx += step)
    grid.push(<line key={"v" + gx} x1={gx} y1={y0} x2={gx} y2={y0 + h} stroke="#232C55" strokeWidth={w * 0.0018} />);
  for (let gy = Math.ceil(y0 / step) * step; gy < y0 + h; gy += step)
    grid.push(<line key={"h" + gy} x1={x0} y1={gy} x2={x0 + w} y2={gy} stroke="#232C55" strokeWidth={w * 0.0018} />);

  const routeColor = msg.method === "BIRD" ? RED : "#7C93E8";
  const dotColor = msg.method === "BIRD" ? RED : "#FFD75E";

  return (
    <svg viewBox={`${x0} ${y0} ${w} ${h}`} className="w-full rounded-xl block" style={{ background: "#161D3E", aspectRatio: "16/9" }}>
      {grid}
      <polyline points={path.map((p) => `${p.x},${p.y}`).join(" ")} fill="none"
        stroke={routeColor} strokeWidth={w * 0.005} strokeDasharray={`${w * 0.014} ${w * 0.011}`}
        strokeLinecap="round" opacity="0.85" />
      {hubs.map((hub) => {
        const p = P(hub);
        return <circle key={hub.id} cx={p.x} cy={p.y} r={R * 0.8} fill="#7C93E8" />;
      })}
      <circle cx={a.x} cy={a.y} r={R} fill="#EDE6D2" />
      <circle cx={b.x} cy={b.y} r={R} fill="none" stroke="#EDE6D2" strokeWidth={R * 0.5} />
      {labels}
      {!state.done && (
        <g>
          <circle cx={cur.x} cy={cur.y} r={R * 2.6} fill="none" stroke={dotColor} strokeWidth={R * 0.35} opacity="0.5">
            <animate attributeName="r" values={`${R * 1.8};${R * 3.6}`} dur="1.6s" repeatCount="indefinite" />
            <animate attributeName="opacity" values="0.6;0" dur="1.6s" repeatCount="indefinite" />
          </circle>
          <circle cx={cur.x} cy={cur.y} r={R * 1.3} fill={dotColor}>
            <animate attributeName="r" values={`${R * 1.2};${R * 1.6};${R * 1.2}`} dur="1.6s" repeatCount="indefinite" />
          </circle>
        </g>
      )}
      {state.done && (
        <g opacity="0.95">
          <circle cx={b.x} cy={b.y} r={F * 1.35} fill="#161D3E" stroke="#EDE6D2" strokeWidth={F * 0.13} />
          <circle cx={b.x} cy={b.y} r={F} fill="none" stroke="#EDE6D2" strokeWidth={F * 0.08} />
          <path d={`M ${b.x - F * 0.45} ${b.y} l ${F * 0.3} ${F * 0.35} l ${F * 0.6} ${-F * 0.7}`}
            stroke="#EDE6D2" strokeWidth={F * 0.18} fill="none" strokeLinecap="round" />
        </g>
      )}
    </svg>
  );
      ...
      <section>
        ...
        <div className="space-y-2">
          <MethodCard
            ...
            icon={<Bird className="w-5 h-5" />} color={RED} title="Rouge-gorge"
        ...
        <div className="p-4 space-y-3">
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="text-[11px] font-mono" style={{ color: "#8A7F5F" }}>{msg.id} · {msg.to.lat.toFixed(4)}, {msg.to.lon.toFixed(4)}</p>
            </div>
...
export default function App() {
  ...
  const inTransit = messages.filter((m) => now < m.plan.deliveredAt).length;
          ...
          <div className="max-w-md mx-auto grid grid-cols-2">
            {[
              ...
              <button key={v} onClick={() => { setView(v); setOpenId(null); }}
                ...
                {v === "track" && inTransit > 0 && (
                  <span className="absolute top-2 right-[28%] min-w-4 h-4 px-1 rounded-full text-[10px] font-bold flex items-center justify-center"
                    style={{ background: BLUE, color: PAPER }}>{inTransit}</span>
                )}
              </button>
            ))}
          </div>
